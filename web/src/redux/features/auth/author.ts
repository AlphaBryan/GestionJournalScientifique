...
import {AuthSlice, LoginRequest, RegisterRequest} from "./slice";
...
export const registerAuthor = createAsyncThunk(
    ...
    async (request: RegisterRequest) => {
        const res = await handleHttpErrors(post('/auth/author/register', request, {noAuth: true}));
        return {
            token: res.headers.get('authorization') as string,
            user: await res.json()
        };
...
export const authorReducers = (builder: ActionReducerMapBuilder<AuthSlice>) => {
    ...
    builder.addCase(registerAuthor.fulfilled, (state, action) => {
        state.authError = false;
        state.token = action.payload.token;
        state.authUser = action.payload.user;
        state.role = 'author';
    ...
    builder.addCase(registerAuthor.rejected, (state, action) => {
        state.authError = true;
    });
}